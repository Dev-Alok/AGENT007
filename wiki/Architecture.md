...
## Technical Stack
- Language: Go 1.22+
- LLM Integrations: Ollama (native JSON RPC)
...
2. **LLM Client (`pkg/llm/client.go`)**:
   Provides an abstraction layer to communicate with Ollama native models. It implements brace-depth tracking logic in `ParseToolCalls` to safely extract natively nested JSON schemas from raw text strings generated by the LLM.
